...
class ZoneTest {
    ...
    @Before
    fun setupZone() {
        list1 = mutableListOf(LatLng(arrayLngLat[0], arrayLngLat[1]),LatLng(arrayLngLat[2], arrayLngLat[3]),LatLng(arrayLngLat[4], arrayLngLat[5]),LatLng(arrayLngLat[6], arrayLngLat[7]))
        list2 = mutableListOf(LatLng(arrayLngLat2[0], arrayLngLat2[1]),LatLng(arrayLngLat2[2], arrayLngLat2[3]),LatLng(arrayLngLat2[4], arrayLngLat2[5]),LatLng(arrayLngLat2[6], arrayLngLat2[7]))
        allCoords.add(list1)
        allCoords.add(list2)
    ...
    fun latitudeLongitudeConversionDoneProperly(){
        ...
        s = s.substring(0,s.length-DatabaseConstant.POINTS_SEP.length)+DatabaseConstant.AREAS_SEP
        for (c in list2) {
            s += c.latitude.toString() + DatabaseConstant.LAT_LONG_SEP + c.longitude + DatabaseConstant.POINTS_SEP
        }
        s = s.substring(0,s.length-DatabaseConstant.POINTS_SEP.length)
        zone.location = s
